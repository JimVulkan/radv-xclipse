...
{
   /* Gated on the hardware feature, not the ISA: Xclipse 920 emits GFX11 encodings on an
    * RDNA2 core, which has no dual-issue VALU. */
   if (!program->dev.has_vopd || program->wave_size != 32)
      return;
