                    ...
                    VkInstance *pInstance)
{
   ...
   /* Earliest proof of life, before any DRM open or ioctl: distinguishes "never loaded" from
    * "loaded and died early". */
   {
      static bool announced = false;
      if (!announced) {
         announced = true;
         AC_XCLIPSE_LOGP(ANDROID_LOG_INFO, "RADV_BUILD",
                             "[ENTRY] radv_CreateInstance pid=%d (ICD loaded, pre-DRM-open)",
                             (int)getpid());
      }
   }
